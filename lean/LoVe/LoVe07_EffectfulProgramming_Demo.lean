...
parameter `α` (i.e., `m α`) and that is equipped with two distinguished
operations:
...
An alternative to `aesop` is `grind`, which is inspired by satisfiability
modulo theories solvers. It uses a combination of reasoning engines working
together to support goals involving equality reasoning, case analysis, and
linear arithmetic, among others. Unlike `simp`, `grind` reasons about
equalities in an unoriented (i.e., not left-to-right) manner. It systematically
derives new equalities from those that are present in the goal. For example, if
the goal contains `b = a` and `f b ≠ f a` as hypotheses, `grind` will derive
`f b = f a` from `b = a` and discover a contradiction with `f b ≠ f a`.
