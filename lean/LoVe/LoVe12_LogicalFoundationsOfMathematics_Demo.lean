...
To force expressions such as `∀n : ℕ, n = n` to be of type `Prop` anyway, we need
a special typing rule for `Prop`:
...
so on, as long as the `match` expression is itself a proof.
